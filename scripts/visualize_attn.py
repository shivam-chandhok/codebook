...
class MyViT(nn.Module):
    def __init__(self, chw, n_patches=7, n_blocks=2, hidden_d=8, n_heads=2, out_d=10):
        ...
        self.blocks = nn.ModuleList([Block(hidden_d, n_heads) for _ in range(n_blocks)])
    ...
    def forward(self, images):
        ...
        x = out.clone()
        print("Input tensor to Transformer (z0): ", x.shape)
        # Transformer Blocks
        ...
        import pdb; pdb.set_trace()
        #q = self.blocks[0].sa.heads[0].query(out)[0].reshape(50, H=2, -1)
        #q = q.permute(H=2, 50, -1)
        #k = self.blocks[0].sa.heads[0].key(out)[0].reshape(50, H=2, -1)
        #k = k.permute(H=2, 50, -1)
        #kT = k.permute(0, 2, 1)
...
def main():
    ...
    model = MyViT(
        (1, 28, 28), n_patches=7, n_blocks=4, hidden_d=768, n_heads=2, out_d=10
    ).to(device)
    N_EPOCHS = 6
    LR = 0.005
